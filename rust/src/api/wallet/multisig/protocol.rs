//! Canonical XELIS wallet multisig request and signature-share protocol.
//!
...
//!
//! See `docs/multisig-signing-protocol.md` for the protocol contract,
//! end-to-end flow, trust boundaries, and transport considerations.
...
const MULTISIG_SIGNING_REQUEST_DOMAIN: &[u8] = b"xelis:wallet:multisig-signing-request:v1\0";
...
/// Canonically decoded signature share used only inside the Rust API.
#[flutter_rust_bridge::frb(ignore)]
#[derive(Clone, Debug)]
pub(super) struct MultisigSignature {
    pub id: u8,
...
impl ParsedMultisigSigningRequest {
    pub fn into_request(
        ...
        signer_id: Option<u8>,
    ) -> NativeMultisigSigningRequest {
        NativeMultisigSigningRequest {
            request_id,
            encoded: self.encoded,
            signing_hash: self.signing_hash.to_hex(),
            source: self.source,
...
pub(super) fn verify_multisig_signature(
    ...
    if !parsed_signature.verify(expected_signing_hash.as_bytes(), &participant) {
...
pub(super) fn create_multisig_signing_request(
...
) -> Result<NativeMultisigSigningRequest> {
    ...
    Ok(parsed.into_request(None, configuration.threshold, participants, None))
}
...
pub(super) fn parse_multisig_signing_request(
    ...
    Ok(ParsedMultisigSigningRequest {
        ...
        signing_hash: unsigned.get_hash_for_multisig(),
        source: transaction
            .get_source()
            .clone()
...
pub(super) fn parse_multisig_signature_share(
    ...
    let envelope: MultisigSignatureShareEnvelope =
        ...
        envelope.signing_hash == expected_signing_hash.to_hex(),
...
fn validate_and_build_transaction_preview(
    ...
    network: Network,
    source: &PublicKey,
) -> Result<NativeMultisigSigningTransaction> {
    match (declared, actual) {
        (
            ...
            for (declared, actual) in declared.iter().zip(actual) {
                ...
                transfers.push(NativeMultisigSigningTransfer {
                    amount: declared.amount,
            ...
            Ok(NativeMultisigSigningTransaction::Transfers { transfers })
        }
        (
            ...
            Ok(NativeMultisigSigningTransaction::Burn {
                asset: actual.asset.to_hex(),
                amount: actual.amount,
            })
        }
        (MultisigSigningRequestTransaction::DeleteMultisig, TransactionType::MultiSig(actual)) => {
            ...
            Ok(NativeMultisigSigningTransaction::DeleteMultisig)
        }
        _ => bail!("The declared transaction type does not match the unsigned transaction"),
    }
}
