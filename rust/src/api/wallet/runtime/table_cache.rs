...
pub(super) async fn get_or_load_precomputed_tables(
    ...
    get_or_load_precomputed_tables_with_policy(precomputed_tables_path, requested_l1, true).await
}

/// Loads or generates tables at the exact requested path even when compatible
/// tables are already cached in memory, then reconciles them with the shared
/// application cache. This preserves update durability across process restarts.
pub(super) async fn materialize_precomputed_tables(
    precomputed_tables_path: &str,
    requested_l1: usize,
) -> Result<PrecomputedTablesShared> {
    get_or_load_precomputed_tables_with_policy(Some(precomputed_tables_path), requested_l1, false)
        .await
}

async fn get_or_load_precomputed_tables_with_policy(
    precomputed_tables_path: Option<&str>,
    requested_l1: usize,
