...
async fn wallet_password_change_persists_and_preserves_data() -> Result<()> {
    ...
    let new_password = "integration-test-new-password";
    ...
    wallet.set_password(password, new_password).await?;
    wallet.close().await;
    drop(wallet);

    assert!(Wallet::open(&wallet_path, password, network, tables.clone(), 1, 1,).is_err());

    let reopened = Wallet::open(&wallet_path, new_password, network, tables, 1, 1)?;
