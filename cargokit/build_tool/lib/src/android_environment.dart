...
class AndroidEnvironment {
  ...
  String _libGccWorkaround(String buildDir, Version ndkVersion) {
    ...
    return buildAndroidRustFlags(
      inheritedRustFlags: Platform.environment['CARGO_ENCODED_RUSTFLAGS'] ?? '',
      workaroundDir: workaroundDir,
      target: target,
    );
  }
}

String buildAndroidRustFlags({
  required String inheritedRustFlags,
  required String workaroundDir,
  required Target target,
}) {
  final encodedArguments = <String>[
    if (inheritedRustFlags.isNotEmpty) inheritedRustFlags,
    '-L',
    workaroundDir,
  ...
  return encodedArguments.join('\x1f');
}
